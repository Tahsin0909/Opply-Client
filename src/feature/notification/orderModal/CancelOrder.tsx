...
const cancellationReasons = [
    ...
    {
        id: "better-deal",
        label: "Found a Better Deal - The customer found the product at a lower price elsewhere.",
    },
    {
        id: "delivery-time",
        label: "Delivery Time Too Long - The estimated delivery time was longer than expected.",
    },
    {
        id: "policy-violation",
        label: "Policy Violation - The order violates marketplace policies (e.g., prohibited items).",
    },
    {
        id: "suspicious-activity",
        label: "Suspicious Activity Detected - The marketplace detects potential fraud.",
    },
    {
        id: "others",
        label: "Others",
...
const CancelOrder = ({ setModalState, samplePost }: { setModalState: React.Dispatch<React.SetStateAction<string>>, samplePost: any }) => {
    ...
    return (
        <div className='p-4'>
            ...
            <div className="mb-6 mt-4">
                ...
                <RadioGroup value={selectedReason} onValueChange={handleReasonChange} className="space-y-3">
                    {cancellationReasons.map((reason) => (
                        <div key={reason.id} className="flex items-center border border-gray-200 rounded-md px-4 py-3">
                            <RadioGroupItem value={reason.id} id={reason.id} className="mr-3" />
                            <label htmlFor={reason.id} className="text-sm text-gray-700 cursor-pointer w-full">
                                {reason.label}
                            </label>
                        </div>
                    ))}
