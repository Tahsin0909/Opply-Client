...
const JobDetails = () => {
  ...
  const handleApply = async () => {
    ...
    try {
      await applyJob({ userId: user.id, jobId }).unwrap();
      toast.success("Successfully applied for the job!");
    } catch (error: any) {
      console.log("Error object:", error); // Log the full error to inspect
      if (
        error?.data?.message === "You have already applied for this job" ||
    ...
    <div className="container section-gap">
      ...
      >
        <div className="rounded-lg bg-primary text-white p-12">
          <div className="flex items-center gap-4">
            <Image
              ...
              src={job.user?.profilePicture || "/default-company.png"}
              alt={job.jobPosition}
            />
        ...
        {job.reqSkills && job.reqSkills.length > 0 && (
          <div className="mt-12 border-b p-4 border-gray-200">
            <Title>Skills</Title>
